...
<section>
    ...
    <c:choose>
        <c:when test="${registerClassVos.rc_approval == -1}">
            <div>승인 대기 중인 회원 입니다.</div>
        ...
        <c:when test="${registerClassVos.rc_approval == 0}">
            <div>신청 거절된 회원 입니다.</div>
            <button type="button" onclick="approve_btn(${registerClassVos.rc_no}, ${registerClassVos.rc_approval})">신청 승인</button>
        </c:when>
        <c:otherwise>
            <div>이미 수강중인 회원 입니다.</div>
        </c:otherwise>
    </c:choose>
